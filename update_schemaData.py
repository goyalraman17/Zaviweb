...
with open("lib/schemaData.ts", "r") as f:
    text = f.read()

# I want to extract out the "faqSchema" code block entirely
pat = re.compile(r"export const faqSchema = \{.*?\n\};\n\n", re.DOTALL)
new_schema = """export const faqSchema = {
...
"""
text = re.sub(pat, new_schema, text)

# Just in case regex fails
if len(text) > 500:
    with open("lib/schemaData.ts", "w") as f:
        f.write(text)
        print("Done")
