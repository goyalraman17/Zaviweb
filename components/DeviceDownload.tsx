...
const platforms: PlatformInfo[] = [
  ...
  },
  {
    name: 'macOS',
    label: 'macOS',
    downloadUrl: DESKTOP_PLATFORM_PAGES.macos,
    icon: (
      <svg className="w-12 h-12" viewBox="0 0 24 24" fill="currentColor">
        <path d="M18.71 19.5C17.88 20.74 17 21.95 15.66 21.97C14.32 22 13.89 21.18 12.37 21.18C10.84 21.18 10.37 21.95 9.09997 22C7.78997 22.05 6.79997 20.68 5.95997 19.47C4.24997 17 2.93997 12.45 4.69997 9.39C5.56997 7.87 7.12997 6.91 8.81997 6.88C10.1 6.86 11.32 7.75 12.11 7.75C12.89 7.75 14.37 6.68 15.92 6.84C16.57 6.87 18.39 7.1 19.56 8.82C19.47 8.88 17.39 10.1 17.41 12.63C17.44 15.65 20.06 16.66 20.09 16.67C20.06 16.74 19.67 18.11 18.71 19.5ZM13 3.5C13.73 2.67 14.94 2.04 15.94 2C16.07 3.17 15.6 4.35 14.9 5.19C14.21 6.04 13.07 6.7 11.95 6.61C11.8 5.46 12.36 4.26 13 3.5Z" />
      </svg>
  ...
  },
  {
    name: 'Linux',
    label: 'Linux',
    downloadUrl: DESKTOP_PLATFORM_PAGES.linux,
    icon: (
      <svg className="w-12 h-12" viewBox="0 0 24 24" fill="currentColor">
...
export default function DeviceDownload() {
  ...
  const [selectedForAccess, setSelectedForAccess] = useState<Platform | null>(
    null
  );
  ...
  const handleAction = (platform: Platform, info: PlatformInfo) => {
    if (info.comingSoon) {
      ...
      handlePlatformDownloadFlow(platform, { fallbackHref: info.downloadUrl });
    }
  };
              ...
              return (
                  ...
                  className={`
                    ...
                    ${
                      isDetected
                        ...
                        : 'bg-gradient-to-br from-slate-50 to-white border-slate-200 text-slate-700 hover:border-blue-300 hover:shadow-lg'
                    }
                  `}
                  whileHover={{ y: -5 }}
                  whileTap={{ scale: 0.98 }}
                >
                  ...
                  <div className="absolute -top-4 left-1/2 transform -translate-x-1/2 whitespace-nowrap z-20">
                    {isDetected ? (
                      <span className="inline-flex items-center gap-1 px-4 py-1.5 bg-blue-600 text-white text-[11px] font-bold uppercase tracking-wider rounded-full shadow-lg border-2 border-white">
                        Your Device
                      </span>
                    ) : (
                      <span className="inline-flex items-center gap-1 px-3 py-1 bg-green-50 text-green-700 text-[10px] font-bold uppercase tracking-wider rounded-full border border-green-200 shadow-sm">
                        Live
                      </span>
                    )}
                  </div>
                  ...
                  <div className="flex flex-col items-center gap-3">
                    <div
                      ...
                      ${isDetected ? 'text-white' : 'text-slate-500 group-hover:text-blue-500'}
                    `}
                    >
                      {platform.icon}
                    </div>
                    ...
                    <span
                      className={`text-lg font-bold ${isDetected ? 'text-white' : 'text-slate-900'}`}
                    >
                      {platform.label}
                    </span>
                    ...
                    <span
                      ...
                      ${isDetected ? 'text-white/90' : 'text-blue-600'}
                    `}
                    >
                      Download
                    </span>
                ...
                {!isSubmitted ? (
                  <>
                    <h3 className="text-xl font-bold text-slate-900 mb-2">
                      Join the Waitlist for {selectedForAccess}
                    </h3>
                    <p className="text-slate-600 mb-6 text-sm">
                      We'll notify you as soon as the {selectedForAccess}{' '}
                      version is ready for preview.
                    </p>
                    <form
