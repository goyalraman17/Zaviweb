...
export default function PlatformDownload() {
  ...
  const handlePrimaryAction = () => {
    const platformNameMap: Record<
      Platform,
      Parameters<typeof handlePlatformDownloadFlow>[0]
    > = {
      android: 'Android',
      ios: 'iOS',
      mac: 'macOS',
    ...
    analytics.track('download_click', {
      ...
      detected_platform: detectedOS || 'unknown',
      is_detected: activePlatform === detectedOS,
    });

    handlePlatformDownloadFlow(platformNameMap[activePlatform], {
      fallbackHref: platformInfo.downloadUrl,
    });
  };
  ...
  return (
    <section
      ...
      className="py-32 md:py-40 bg-white border-y border-zavi-border"
    >
            ...
            {(Object.entries(platforms) as [Platform, PlatformConfig][]).map(
              ([key, platform]) => (
                <motion.button
                  ...
                  transition={{
                    duration: 0.4,
                    delay: 0.1 * Object.keys(platforms).indexOf(key),
                  }}
                >
                  <div
