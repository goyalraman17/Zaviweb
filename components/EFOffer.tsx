...
const OFFER_LABEL = 'Entrepreneur First member offer';
const OFFER_DETAIL = '3 months of Pro at 50% off';

const proofPoints = [
  'Draft investor updates, docs, follow-ups, and team messages by voice',
  'Turn rough thinking into polished writing without slowing down',
...
export default function EFOffer() {
  ...
  const startCheckout = async () => {
    ...
    setPaymentStep('Preparing your EF offer...');
    ...
    try {
      ...
      const response = await fetch('/api/razorpay/order', {
        ...
        body: JSON.stringify({
          ...
          email: trimmed,
          // Partner offer activation is currently keyed off this existing source.
          source: 'f6s',
      ...
      if (!response.ok) {
        throw new Error(
          data?.error || 'Checkout is unavailable in this static build right now.'
        );
      }

      if (!data.orderId) {
        throw new Error(data.error || 'Unable to create the EF payment right now.');
      }
      ...
      const paymentObject = new (window as any).Razorpay({
        ...
        currency: 'USD',
        name: 'Zavi AI',
        description: 'Entrepreneur First Member Offer - 3 Months of Pro at 50% Off',
        ...
        handler: async function (checkoutResponse: any) {
          try {
            ...
            if (verifyRes.ok && result.success) {
              ...
              setResultMessage(
                'Your Entrepreneur First 3-month Pro pass is now active. Open Zavi and start writing faster.'
              );
              setResultPaymentId(
          ...
          <div className="grid gap-8 lg:grid-cols-[1.15fr_0.85fr] lg:items-start lg:gap-10">
            <div>
              <div className="mb-4 flex flex-wrap items-center gap-2 sm:mb-5 sm:gap-3">
                <div className="rounded-full border border-blue-200 bg-white/90 px-4 py-2 text-xs font-semibold text-blue-700 shadow-sm sm:text-sm">
                  {OFFER_LABEL}
                </div>
                <div className="rounded-full border border-amber-200 bg-amber-50 px-4 py-2 text-xs font-semibold text-amber-800 shadow-sm sm:text-sm">
                  {OFFER_DETAIL}
                </div>
              </div>

              <h1 className="max-w-3xl text-4xl font-black leading-[0.95] tracking-tight text-slate-950 sm:text-5xl lg:text-6xl">
                Turn founder-speed thinking into finished work.
              </h1>
              <p className="mt-4 max-w-2xl text-base leading-7 text-slate-700 sm:mt-5 sm:text-lg sm:leading-8 xl:text-xl">
                Speak naturally. Get polished writing across email, docs, chat, and forms.
                Entrepreneur First members get 3 months of Zavi Pro at 50% off with one
                payment and no auto-renewal.
              </p>
              ...
              <div className="mt-8 rounded-[1.75rem] border border-slate-200 bg-white/90 p-5 shadow-lg shadow-blue-100/60 sm:mt-10 sm:rounded-[2rem] sm:p-6">
                ...
                <div className="mt-4 flex flex-col items-start gap-1 sm:flex-row sm:flex-wrap sm:items-end sm:gap-4">
                  <div className="text-5xl font-black text-slate-950 sm:text-6xl">
                    ${OFFER_PRICE.toFixed(2)}
                  </div>
                  <div className="text-base text-slate-600 sm:pb-2 sm:text-lg">
                    for 3 months of Pro
                  </div>
                </div>
                <p className="mt-4 max-w-2xl text-sm leading-6 text-slate-600">
                  This is a one-time Entrepreneur First payment for 3 months of Pro access. It
                  does not auto-renew. You get the discount now, full access immediately after
                  activation, and the freedom to decide later whether you want more.
                </p>
              </div>
            </div>
              ...
              <div className="rounded-[1.55rem] bg-white p-5 sm:rounded-[1.85rem] sm:p-8">
                <div className="mb-5 flex flex-col gap-4 sm:mb-6 sm:flex-row sm:items-center sm:justify-between">
                  ...
                  <div className="w-full rounded-2xl bg-amber-100 px-4 py-3 text-left sm:w-auto sm:text-right">
                    ...
                    <div className="text-sm text-amber-900">instead of ${FULL_PRICE.toFixed(2)}</div>
                  </div>
                </div>
                ...
                <div className="mt-6 rounded-2xl border border-blue-100 bg-blue-50 px-4 py-4 text-sm leading-6 text-slate-700">
                  <span className="font-semibold text-slate-950">Simple promise:</span> one payment,
                  3 months of Pro, no surprise renewal, and activation tied to your Zavi account
                  email.
                </div>
              </div>
            </motion.div>
